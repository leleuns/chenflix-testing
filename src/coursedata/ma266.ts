export const ma266_data: Course = {
	id: 5,
	path: "/ma266",
	abbreviation: "ma26600",
	name: "MA 26600",
	semester: "EDIT HERE",
	description: "Ordinary Differential Equations",
	events: [
		{
			description: "Lesson 1 (1.1 Basic Models and Direction Fields)",
			date: "6/13",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-13_P/0_b9yvuzcn/96165051",
		},
		{
			description:
				"Lesson 2 (1.2 Solutions of Some DEs and 1.3 Classifications of DEs)",
			date: "6/14",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-14_P/0_2zsxwuvu/96165051",
		},
		{
			description: "Lesson 3 (2.1 Linear equations, Integrating Factors)",
			date: "6/15",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-15_P/0_sdeyh19y/96165051",
		},
		{
			description: "Lesson 4 (2.2 Separable Equations)",
			date: "6/16",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-16_P/0_z8cyl2gb/96165051",
		},
		{
			description: "Lesson 5 (2.3 Modeling with First Order Equations)",
			date: "6/17",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-17_P/0_xbmlx7ia/96165051",
		},
		{
			description:
				"Lesson 6 (2.4 Differences Between Linear and Nonlinear Equations)",
			date: "6/20",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-20_P/0_fuw0kpnu/96165051",
		},
		{
			description: "Lesson 7 (2.5 Autonomous Equations)",
			date: "6/21",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-21_P/0_o4y7it21/96165051",
		},
		{
			description: "Lesson 8 (2.6 Exact Equations)",
			date: "6/22",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-22_P/0_khyz0zot/96165051",
		},
		{
			description: "Lesson 9 (2.7 Euler's Method)",
			date: "6/23",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-23_P/0_0oi9c1vg/96165051",
		},
		{
			description:
				"Lesson 10 (3.1 2nd Order Homogeneous Eqs w/ Constant Coefficients)",
			date: "6/24",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-24_P/0_pazdnzsx/96165051",
		},
		{
			description: "Exam 1 Review",
			date: "6/20",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-27_P/0_qflgqd14/96165051",
		},
		{
			description: "Lesson 11 (3.2 Solutions of Linear Homogeneous Eqs))",
			date: "6/29",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-29_P/0_t67gvwvx/96165051",
		},
		{
			description: "Lesson 12 (3.3 Complex Roots of Characteristic Eq.)",
			date: "6/30",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-06-30_P/0_fkeggx6s/96165051",
		},
		{
			description: "Lesson 13 (3.4 Repeated Roots; Reduction of Order)",
			date: "7/01",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-01_P/0_7be7ruby/96165051",
		},
		{
			description:
				"Lesson 14 (3.5 Nonhomogeneous Eqs: Method of Undetermined Coefficients)",
			date: "7/05",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-05_P/0_ev3agaz0/96165051",
		},
		{
			description: "Lesson 15 (3.6 Variation of Parameters)",
			date: "7/06",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-06_P/0_jbz5iufl/96165051",
		},
		{
			description: "Lesson 16 (3.7 Mechanical Vibrations)",
			date: "7/07",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-07_P/0_nx78qgow/96165051",
		},
		{
			description: "Lesson 17 (3.8 Forced Vibrations)",
			date: "7/08",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-08_P/0_gqu9rfvt/96165051",
		},
		{
			description: "Lesson 18 (4.1 + 4.2 Higher-Order Linear Equations)",
			date: "7/11",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-11_P/0_1pcvf04s/96165051",
		},
		{
			description: "Lesson 19 (4.3 The Method of Undetermined Coefficients)",
			date: "7/12",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-12_P/0_vblvr27t/96165051",
		},
		{
			description: "Lesson 20 (6.1 The Laplace Transform)",
			date: "7/13",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-13_P/0_ce3x57t5/96165051",
		},
		{
			description: "Exam 2 Review",
			date: "7/14",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-14_P/0_2akvcxt0/96165051",
		},
		{
			description: "Lesson 21 (6.2 Solutions of Initial-Value Problems)",
			date: "7/18",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-18_P/0_eo9giokh/96165051",
		},
		{
			description: "Lesson 22 (6.3 Step Functions)",
			date: "7/19",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-19_P/0_9e92gxpk/96165051",
		},
		{
			description: "Lesson 23 (6.4 DE's with Discontinuous Forcing Functions)",
			date: "7/20",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-20_P/0_bw5pppvr/96165051",
		},
		{
			description: "Lesson 24 (6.5 Impulse Function)",
			date: "7/21",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-21_P/0_dhbdm17u/96165051",
		},
		{
			description: "Lesson 25 (6.6 The Convolution Integral)",
			date: "7/22",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-22_P/0_wsac4ycb/96165051",
		},
		{
			description: "Lesson 26 (7.1 + 7.2 Systems of First Order Linear Eqs)",
			date: "7/25",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-25_P/0_f1b7gr5b/96165051",
		},
		{
			description:
				"Lesson 27 (7.3 Linear Independence, Eigenvalues, Eigenvectors)",
			date: "7/26",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-26_P/0_2vbrl4hd/96165051",
		},
		{
			description:
				"Lesson 28 (7.5 Homogeneous Systems with Constant Coefficients )",
			date: "7/27",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-27_P/0_9mok5kj5/96165051",
		},
		{
			description: "Lesson 29 (7.6 Complex Eigenvalues)",
			date: "7/28",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-28_P/0_yvvf6dud/96165051",
		},
		{
			description: "Lesson 30 (7.8 Repeated Eigenvalues)",
			date: "7/29",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-07-29_P/0_4ba2ycld/96165051",
		},
		{
			description: "Lesson 31 (7.9 Nonhomogeneous Systems)",
			date: "8/01",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-08-01_P/0_eb6yt6nm/96165051",
		},
		{
			description: "Final Exam Review",
			date: "8/02",
			videos:
				"https://mediaspace.itap.purdue.edu/media/Summer+2016+-+MA266+-+Chen_2016-08-02_P/0_e7e7g64u/96165051",
		},
	],
};
